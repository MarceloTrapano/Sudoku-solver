...
class Controller:
    ...
    def __init__(self, config, sudoku, predef, model) -> None:
        ...
        self.model: Model = model # Create model

        self.in_try: bool = False # Detect pressed 'try' state
        self.highlight: bool = True # Detect if sudoku is highlighted

    @staticmethod
    def _change_language(list_of_elements: List[Any], list_of_prompts: List[str], state: bool) -> None:
    ...
    def switch_event(self, state: bool) -> None:
        ...
        Args:
            state (bool): 0-English, 1-Polish.
        """
        self._change_language([self.config.scan_button, self.predef.id_button], 
        ...
        if self.config.in_try:
            self._change_language([self.config.try_button, self.config.check_button, self.config.solve_button],
    ...
    def solve_sudoku(self, name: str, state: bool) -> None | bool:
        ...
        try:
        ...
        except ValueError: # Catch error when sudoku is not correct
            ...
            return False
        if not state: # Display estiated time and solved sudoku
            text: List[str] =  [f"Solved in {time:0.4f} s.", f"Rozwiązano w {time:0.4f} s."]
            self.config.set_textbox(text)
            self.sudoku.insert(solution, state="disabled")
    ...
    def check_sudoku(self) -> None:
        ...
        if not check.any():
            ...
            # If sudoku was solved correctly, display message
        if self.highlight:
    ...
    def load_sudoku(self, id_num: int | str) -> None:
        ...
        except ValueError: # Catch error when loading sudoku
